import React, { useState } from "react";
import {
    MDBBtn,
    MDBIcon,
    MDBInput, MDBAnimation
} from "mdbreact";
import emailjs from "emailjs-com";
import Swal from "sweetalert2";
import imgBody from "../img/imgBody.png";
import "./contact.css";


const Contact = () => {

    const [contact, setContac] = useState({
        name: "",
        email: "",
        tema: "",
        mensaje: "",
    });

    const handleChangeInput = (e) => {
        setContac({ ...contact, [e.target.name]: e.target.value });
    };

    const disabledBoton =
        contact.name.length < 3 ||
        contact.email.length < 5 ||
        contact.tema.length < 3 ||
        contact.mensaje.length < 5;

    function sendForm(e) {
        e.preventDefault();
        emailjs.sendForm(
            "capezGmail",
            "template_J1gciyhd",
            e.target,
            "user_ohXjZB0I0Z5ClNvVUWGWd"
        ).then((result) => {
            setContac({ name: "", email: "", tema: "", mensaje: "" });
            Swal.fire(
                "¡Genial!",
                "Tu consulta fue enviada correctamente",
                "success"
            );
        }, (error) => {
            alert("Por favor complete los campos");
        });
    }

    return (
        <div className="contacto-container">
            <div className="contacto-form">
                <MDBAnimation reveal type="zoomIn" duration="1s">
                    <form onSubmit={sendForm} style={{}}>
                        <p className="h5 mb-4">
                            Hacé tu consulta ¡Estamos para ayudarte!
                        </p>
                        <div className="grey-text">
                            <MDBInput
                                label="Tu nombre"
                                icon="user"
                                group
                                type="text"
                                validate
                                error="wrong"
                                success="right"
                                onChange={(e) => handleChangeInput(e)}
                                name="name"
                                value={contact.name}
                                required
                            />
                            <MDBInput
                                label="Tu email"
                                icon="envelope"
                                group
                                type="email"
                                validate
                                error="wrong"
                                success="right"
                                name="email"
                                onChange={(e) => handleChangeInput(e)}
                                value={contact.email}
                                required
                            />
                            <MDBInput
                                label="Tema"
                                icon="tag"
                                group
                                type="text"
                                validate
                                error="wrong"
                                success="right"
                                name="tema"
                                onChange={(e) => handleChangeInput(e)}
                                value={contact.tema}
                                required
                            />
                            <MDBInput
                                type="textarea"
                                rows="2"
                                label="Tu mensaje"
                                icon="pencil-alt"
                                name="mensaje"
                                onChange={(e) => handleChangeInput(e)}
                                value={contact.mensaje}
                                required
                            />
                        </div>
                        <div className="text-center">
                            <MDBBtn
                                type="submit"
                                value="send"
                                outline={disabledBoton}
                                color="success"
                                disabled={disabledBoton} >
                                Enviar
                                <MDBIcon far icon="paper-plane" className="ml-1" />
                            </MDBBtn>
                        </div>
                    </form></MDBAnimation>
            </div>
            <div className="col-sm-12 col-md-6 contacto-image">
                <img src={imgBody} 
                    alt="body img" 
                    style={{ maxWidth: "100%" }}>
                </img>
            </div>
        </div>
    );
};

export default Contact;
